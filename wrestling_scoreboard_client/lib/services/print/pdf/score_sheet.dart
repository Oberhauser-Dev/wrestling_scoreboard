...
class ScoreSheet extends PdfSheet {
    ...
    if (bout.winnerRole != null && bout.result != null) {
      resultRule = BoutConfig.resultRule(
        ...
        technicalPointsLoser: AthleteBoutState.getTechnicalPoints(
          boutActions,
          bout.winnerRole == BoutRole.red ? BoutRole.blue : BoutRole.red,
        ),
        rules: boutRules,
      );
    }
  ...
  Widget _buildClassificationPointsTableSection(
    Context context,
    Map<(BoutResult, int, int), List<BoutResultRule>> boutResultRuleGroups,
    BoutResultRule? resultRule,
  ) {
  ...
  Widget _buildPointsBody(Context context, List<BoutAction> actions) {
    ...
    Widget buildTotalCell(PdfColor borderColor) => buildTextCell(
      localizations.total.toUpperCase(),
      height: headerCellHeight,
      borderColor: borderColor,
      alignment: Alignment.center,
    );
    Widget buildTechnicalPointsHeaderCell(PdfColor borderColor) => buildTextCell(
      localizations.technicalPoints.toUpperCase(),
      height: headerCellHeight,
      borderColor: borderColor,
      alignment: Alignment.center,
    );
    ...
    TableRow buildRound({required int round}) {
      ...
      return TableRow(
        children: [
          ...
          buildTextCell(
            '${localizations.round.toUpperCase()}\n${round + 1}',
            height: roundCellHeight,
      ...
      children: [
        TableRow(
          children: [
            TableCell(
              rowSpan: 2 * rounds + 1,
              child: buildColorCell(colorStr: localizations.red.toUpperCase(), borderColor: PdfSheet.homeColor),
            ),
            ...
            TableCell(
              rowSpan: 2 * rounds + 1,
              child: buildColorCell(colorStr: localizations.blue.toUpperCase(), borderColor: PdfSheet.guestColor),
            ),
          ],
