...
class LocalDataNotifier<T extends DataObject> extends _$LocalDataNotifier<T> {
  ...
  Raw<Future<List<T>>> build() async {
    ...
    final defaultEntities = switch (T) {
    ...
    if (diff.isNotEmpty) {
      ...
      result.addAll(diff.map((id) => defaultEntities.singleWhere((e) => e.id == id)));
    }
  ...
  Future<void> setState(List<T>? val) async {
    ...
    final dataAsStr = val?.map((e) => jsonEncode(e.toRaw())).toList();
